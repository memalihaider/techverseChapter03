...
const team2026 = [
  { name: 'Muhammad Sheharyar Shahzad Rana', role: 'President', imageUrl: 'https://drive.google.com/uc?export=view&id=1CxENenoCq4o7ww8CYOtvAiNwVP2X-wBX' },
  { name: 'HUSNAIN MEHMOOD', role: 'Vice President', imageUrl: 'https://drive.google.com/uc?export=view&id=1ctyX4-b0uSzIRZB_Hgww61faPciLYCgQ' },
  { name: 'Muhammad Ali Haider', role: 'Secretary General', imageUrl: 'https://drive.google.com/uc?export=view&id=1cCw75b1lHOByQRmQGIIMGcmeKS8US8u0' },
  { name: 'Abdul Wahab', role: 'Secretary General', imageUrl: 'https://drive.google.com/uc?export=view&id=1TaXHM1Ixhkma4Oa6L9z1MasXgaO6h160' },
  { name: 'Muhammad Saad Ahmed', role: 'Media Secretary', imageUrl: 'https://drive.google.com/uc?export=view&id=1nNZxprhaIm8w228yUbhEMs8bSZo_bYmS' },
  ...
  { name: 'Ms. Khadija', role: 'Female Vice President', imageUrl: '' },
...
const president = team2026.filter(m => m.role.toLowerCase().includes('president'));
const executives = team2026.filter(m =>
...
const directors = team2026.filter(m => m.role.toLowerCase().includes('director') || m.role.toLowerCase().includes('lead') || m.role.toLowerCase().includes('head') || m.role.toLowerCase().includes('media secretary') || m.role.toLowerCase().includes('module head'));
const supervisors = team2026.filter(m => m.role.toLowerCase().includes('supervisor'));

function TeamGroup({ title, members, onImageClick }: { title: string, members: typeof team2026, onImageClick: (url: string, name: string) => void }) {
    ...
    <div className="min-h-screen bg-gradient-to-br from-blue-50 to-navy-50 py-16">
      <div className="max-w-5xl mx-auto px-4">
        ...
        <p className="text-center text-lg text-navy-700 mb-12">
          Meet the legacy team that made Techverse 2026 a success!
        </p>
